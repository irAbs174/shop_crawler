...
def send_message(server_socket):
    while True:
        try:
            ...
            elif 'sitemap_to_crawl=>' in response:
            ...
            elif 'get_product_info=>' in response:
                product_url = response.split('=>')[1]
                info = get_product_info(product_url)
                name = info['name']
                price = info['price']
                stock = info['status']
                server_socket.send(f"product_info=>{name}=>{price}=>{stock}=>{product_url}".encode('utf-8'))
